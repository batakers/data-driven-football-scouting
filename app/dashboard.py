...
@st.cache_data
def load_explainability_outputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    """Load precomputed SHAP outputs generated by src/explainability.py."""
    ...
    details_path = output_dir / "player_explanations" / "player_explanation_table.csv"
    ...
    return importance, candidates, details, report

def explainability_image_path(filename: str) -> str | None:
    path = Path("outputs/explainability") / filename
    return str(path) if path.exists() else None

def player_waterfall_path(player_id) -> str | None:
    try:
        player_id = int(player_id)
    except (TypeError, ValueError):
        return None
    path = Path("outputs/explainability/player_explanations") / f"player_{player_id}_waterfall.png"
    return str(path) if path.exists() else None

def explainability_feature_label(feature: str) -> str:
    labels = {
        "age_at_valuation": "Age",
        "minutes_last_season": "Minutes Played",
        "goals_per_90_ls": "Goals / 90",
...
tab_scouting, tab_similarity, tab_explainability = st.tabs([
    "💎 Scouting Overview",
    "🔍 Similarity Search",
    ...
    if engine is None:
        ...
        with col_s2:
            role_mode_label = st.selectbox(
                ...
                index=0,
                help="Compatible Roles uses the role compatibility matrix. Exact Role requires the same primary role. Broad Position Group falls back to position-group behavior."
            )
            role_mode_map = {
...
with tab_explainability:
    ...
    if importance_df.empty or explanation_candidates_df.empty:
        ...
        model_options = {
            "Performance-only Model A": {
                "label": "Model A - Performance Only",
                "bar": "shap_bar_model_a.png",
                "summary": "shap_summary_model_a.png",
                "description": "Model A is the scouting model. It avoids previous market value and explains discovery signals from performance, age, position, and profile features.",
            },
            "Market-aware Model B": {
                "label": "Model B - Market Aware",
                "bar": "shap_bar_model_b.png",
                "summary": "shap_summary_model_b.png",
                "description": "Model B is the benchmark model. It includes previous market value, so its explanations often show market inertia.",
            },
        }
